...
public:
    AsciiWidget(QWidget *parent = nullptr) : QOpenGLWidget(parent) {
        srand(static_cast<unsigned int>(time(nullptr))); // Seed for random color generation
    ...
    void paintGL() override {
        ...
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                ...
                painter.setBrush(data.color); // Set the brush to fill the square with color
                ...
                painter.drawText(rect, Qt::AlignCenter, QString(QChar(rand() % 95 + 32))); // Random ASCII character
            }
        }
    }
    ...
    void generateAsciiData() {
        ...
        for (int i = 0; i < rows; ++i) {
            for (int j = 0; j < cols; ++j) {
                ...
                if (asciiCode > 126) break; // Stop if beyond printable ASCII
